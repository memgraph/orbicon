...
class Memgraph:
    ...
    def execute_query(self, query: str, connection: Optional[Connection] = None) -> None:
    ...
    def create_constraint(self, index: Union[MemgraphConstraintExists, MemgraphConstraintUnique]) -> None:
        """Creates a constraint (label or label-property type) in the database"""
        query = f"CREATE CONSTRAINT ON {index.to_cypher()}"
        self.execute_query(query)

    def drop_constraint(self, index: Union[MemgraphConstraintExists, MemgraphConstraintUnique]) -> None:
    ...
    def get_constraints(self) -> List[Union[MemgraphConstraintExists, MemgraphConstraintUnique]]:
        ...
        for result in self.execute_and_fetch("SHOW CONSTRAINT INFO"):
            ...
            elif result[MemgraphConstants.CONSTRAINT_TYPE] == MemgraphConstants.CONSTRAINT_TYPE_EXISTS:
                constraints.append(
